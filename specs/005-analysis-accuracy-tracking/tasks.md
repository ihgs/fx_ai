...
- [ ] 6. `lib/judgeAnalysis.ts` の判定ロジックを手動検証する（`rate_candles` にテストデータを投入し `GET /api/analysis` のレスポンスで正解/不正解/判定待ちが期待通りになるか確認）。UIは `ui-check` で対象storyを目視確認する（Design: Testing Approach）
