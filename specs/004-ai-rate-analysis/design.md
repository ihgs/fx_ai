...
| モデル | `gemini-3.6-flash` | 定期実行（1日16回）+ 手動実行があるためコストを抑える目的で軽量なflashモデルを使う（分類寄りの軽いタスクのため）。`-latest`系の浮動エイリアスは混雑しやすく、また`gemini-2.5-flash`は新規ユーザー向けに提供終了（404）だったため、Google推奨の`gemini-3.6-flash`に固定している。将来別モデルにしたい場合は`lib/analyzeRate.ts`内の`MODEL`定数1箇所を変えるだけで済む。 |
