...
| 用途 | 選定 | 理由 |
|---|---|---|
| AI API | `@anthropic-ai/sdk`（公式TypeScript SDK） | Claude Code自身と同じAnthropic APIを使う。`ANTHROPIC_API_KEY`環境変数を`.env.local`に設定する必要がある（現時点で未取得。取得までは分析実行がエラーになるが、Req 1.4のエラー表示で許容する）。 |
| モデル | `claude-opus-5` | `claude-api`スキルの既定ルールに従う。定期実行（1日16回）+ 手動実行があるためコストを抑える目的で`output_config.effort: "low"`を指定する（分類寄りの軽いタスクのため）。将来コストが気になる場合は`lib/analyzeRate.ts`内の1箇所を変えるだけで済む。 |
| 構造化出力 | `client.messages.parse()` + `zodOutputFormat()`（`@anthropic-ai/sdk/helpers/zod`） | 見通し（up/down/flat）と根拠をJSONで確実に受け取るため。強制tool_choiceより現行の推奨方式。 |
...
- `lib/analyzeRate.ts`: `runAnalysis(trigger: "manual" | "scheduled")` を実装。
  ...
  2. 履歴をプロンプトに整形し、`client.messages.parse()` + Zodスキーマ（`direction: "up"|"down"|"flat"`, `rationale: string`）で構造化出力を取得。
...
- `POST /api/analysis`
  ...
  - 502: `{ error: string }`（レート履歴が無い／Anthropic API呼び出し失敗／DB書き込み失敗のいずれか）
...
- `package.json`（`@anthropic-ai/sdk`, `zod`を追加）
- `.env.local`（`ANTHROPIC_API_KEY`。gitignore済みの`.env*`パターンでコミット対象外）

## Error Handling / Edge Cases
- レート履歴が0件の場合、分析を実行せずエラーを投げる（手動実行なら502、定期実行なら`console.error`のみでスケジュール継続）。
- Anthropic API呼び出し失敗（認証エラー含む）・構造化出力のパース失敗・DB書き込み失敗はすべて同様に扱う。
...
- `ANTHROPIC_API_KEY`が未設定の間は実際のAI呼び出しは検証できない。その間はエラー表示経路（Req 1.4）が正しく動くことと、DB層（`insertAnalysisResult`/`getAnalysisResults`/`getRecentCandles`）が単体で正しく動くことを手動確認する。キー取得後に実際の分析実行をE2Eで確認する。
