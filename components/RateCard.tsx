...
export function RateCard(props: RateCardProps) {
  return (
    <div
      className="w-full rounded-2xl bg-white p-6 shadow-sm ring-1 ring-black/5
    ...
    >
      {props.status === "loading" && (
        <div role="status" aria-live="polite" className="animate-pulse space-y-4 landscape:flex-1">
          <div className="h-5 w-28 rounded bg-zinc-200 portrait:mx-auto" />
          <div className="h-16 w-64 rounded bg-zinc-200 portrait:mx-auto" />
          <div className="h-4 w-48 rounded bg-zinc-200 portrait:mx-auto" />
        </div>
      )}
      ...
      {props.status === "error" && (
        <div role="alert" className="landscape:flex-1">
          ...
          <p className="mt-1 text-base text-zinc-500">{props.message}</p>
        </div>
      )}

      {props.status === "loaded" && (
        <div className="landscape:flex-1">
          <p className="text-lg font-medium text-zinc-500">USD/JPY</p>
          <p className="mt-1 text-7xl font-semibold tabular-nums text-zinc-900">
